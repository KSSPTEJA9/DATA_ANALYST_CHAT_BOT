...
def main():
    ...
    if user_message:
        ...
        # # Reset the input field for the next question by clearing the session state value
        # st.session_state.user_message = ""  # Clear the input field in session state

        # # Immediately reset the input box value as well (this will clear the text input)
        # # Use a unique key for the text_input to avoid duplicate element ID errors
        # st.text_input("You: ", value=st.session_state.get("user_message", ""), key="user_input_field")
